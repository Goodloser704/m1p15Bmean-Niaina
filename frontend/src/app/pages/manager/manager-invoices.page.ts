...
@Component({
    ...
    <div class="page-mechanic-theme">
      <div class="wrap">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-card">
            ...
            <div class="stat-content">
              ...
              <div class="stat-label">Factures à générer</div>
            </div>
          </div>
        </div>
        ...
        <div class="card" *ngIf="paidWorkOrdersWithoutInvoice().length > 0">
          ...
          <p class="info">
            {{ paidWorkOrdersWithoutInvoice().length }} réparation(s) payée(s) sans facture.
          </p>
          <button (click)="generateAllPendingInvoices()" [disabled]="generating()" class="generate-all-btn">
          ...
          <div *ngIf="invoices().length > 0" class="invoices-list">
            <div *ngFor="let invoice of invoices()" class="invoice-item">
              ...
              <div class="invoice-actions">
                ...
                <button (click)="sendInvoice(invoice)" class="send-btn" *ngIf="invoice.status === 'draft'">
                  📧 Envoyer
                </button>
              </div>
            </div>
        ...
        <!-- Modal détail facture (même que client) -->
            ...
            <div class="invoice-detail">
              <div class="invoice-header-detail">
                ...
                <div class="invoice-info">
                  ...
                  <span class="status" [class]="'status-' + selectedInvoice()!.status">
  ...
  styles: [`
    ...
    /* Styles modal identiques à client-invoices */
    .modal {
      position: fixed;
      top: 0;
      left: 0;
...
export class ManagerInvoicesPageComponent {
  ...
  totalRevenue() {
    return Math.round(this.invoices().reduce((sum, inv) => sum + inv.totalTTC, 0) * 100) / 100;
  }

  totalVAT() {
    return Math.round(this.invoices().reduce((sum, inv) => sum + inv.totalVAT, 0) * 100) / 100;
  }

  pendingInvoices() {
    return this.paidWorkOrdersWithoutInvoice().length;
  }

  async generateAllPendingInvoices(): Promise<void> {
    this.generating.set(true);
    this.error.set(null);

    try {
      const pendingWorkOrders = this.paidWorkOrdersWithoutInvoice();
      let generated = 0;

      for (const wo of pendingWorkOrders) {
        try {
          await this.invoicesService.generateFromWorkOrder(wo._id);
          generated++;
        } catch (error) {
          console.error(`Erreur génération facture pour WO ${wo._id}:`, error);
        }
      }

      this.success.set(`${generated} facture(s) générée(s) avec succès !`);
      await this.loadData();
    } catch (error: any) {
      this.error.set('Erreur lors de la génération des factures');
    } finally {
  ...
  sendInvoice(invoice: Invoice): void {
    ...
    this.success.set('Envoi par email à implémenter');
  }
}